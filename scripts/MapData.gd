...
static func _rooftops() -> Dictionary:
	return {
		...
		"enemies": {
			# stage 0~1 등장 — 드론 제거. snipers는 상층 옥상 sniper post에서 위에서 LoS.
			# 사용자 피드백 "저격수가 저격수다운 위치에" + "상단 1/3 콘텐츠 없음" 동시 해소.
			"patrol": [Vector2(640, 2650.0), Vector2(640, 2010.0), Vector2(560, 1670.0)],
			"sniper": [
				Vector2(660, 1190.0),  # ROOF 5 sniper post — 중상층 LoS
				Vector2(560, 710.0),   # ROOF 6 sniper post — 상층 LoS (탑처럼 내려다봄)
			],
			"drone":  [],
			"bomber": [], "shield": [],
		},
...
static func _watchtower() -> Dictionary:
	return {
		...
		"enemies": {
			# 감시탑 = sniper 컨셉. 사용자 피드백 "저격수다운 위치에" — 분기 끝(저격선이
			# 짧음)이 아닌 상층 sniper post에서 아래로 LoS 확보. 외부 분기에도 1명 유지.
			"patrol": [Vector2(700, 2810.0), Vector2(280, 1950.0), Vector2(540, 1950.0)],
			"sniper": [
				Vector2(280, 1950.0),  # 외부 분기 — patrol 자리와 같이 (XP 보호)
				Vector2(660, 1150.0),  # 중층 정찰단 (sniper post 1)
				Vector2(560, 670.0),   # 정상 정찰단 (sniper post 2)
			],
			"drone":  [],
			"bomber": [],
			"shield": [],
...
static func _blackout() -> Dictionary:
	return {
		...
		# (이전엔 660이라 지면 안에 묻혀 보이지 않고 데미지도 안 들어가던 버그)
		# x는 발판 사이 갭에 맞춤 — 지면 보행을 강제로 끊어 정밀 점프 강요.
		"spikes": [
			{"x": 480.0, "y": 594.0, "w": 100.0},   # 320(540)와 560(480) 사이
			{"x": 950.0, "y": 594.0, "w": 100.0},   # 820(520)와 1080(460) 사이
			{"x": 1500.0, "y": 594.0, "w": 100.0},  # 1340(520)와 1620(480) 사이
			{"x": 2050.0, "y": 594.0, "w": 100.0},  # 1900(540)와 2160(520) 사이
		],
