...
static func _rooftops() -> Dictionary:
	return {
		...
		"platforms": [
			...
			# 글라이드 게이트 — 비밀 옥상 끝(2280) 위 220px 단독 알코브. 더블점프(190)론 못 닿고
			# 삼단점프(글라이드 T1, 381)로만. 비밀을 찾은 사람만 보는 "더 깊은 보상". 위 XP가 보여 유도.
			{"pos": Vector2(1180, 2060), "w": 90.0},
...
static func _cooling() -> Dictionary:
	return {
		...
		"platforms": [
			...
			# 저격 둥지 — 좌측 분기 밖 가장자리 단독 발판. 분기 climber를 사선으로 견제(올라설 필요 없음).
			{"pos": Vector2(40, 1620), "w": 56.0},
