...
static func _cooling() -> Dictionary:
	return {
		...
		"platforms": [
			...
			# 글라이드 게이트 — 비밀 끝(2480) 위 230px 보너스 알코브. 더블점프(apex 2290)론 못 닿고
			# 삼단점프(글라이드 T1, apex 2210)로만. 비밀을 찾은 사람만 보는 "더 깊은 보상".
			{"pos": Vector2(1100, 2250), "w": 100.0},
...
static func _ward() -> Dictionary:
	return {
		...
		"platforms": [
			...
			# 못 닿고 삼단점프(글라이드 T1, apex 150)로만 닿음. 위 보상이 보여 "어떻게 올라가지?" 유도.
